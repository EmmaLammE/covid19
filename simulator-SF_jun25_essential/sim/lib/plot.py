...
class Plotter(object):
    ...
    def compare_total_infections_essenprop(self, sims, titles, figtitle='Title',
        ...
        show_positives=False, show_legend=True, legendYoffset=0.0, legend_is_left=True,
        subplot_adjust=None, start_date='2020-03-04', first_one_dashed=False, counts=None,ax=None,fig=None):
...
#         ax = fig.add_subplot(111)
        ...
        for i in range(len(sims)):
            ...
            # lines
            if show_positives:
                ax.errorbar(ts, line_infected, yerr=error_infected, label='[Infected] ' + titles, errorevery=errorevery,
                           c=self.color_different_scenarios[counts], linestyle='-')

                T = posi_mu.shape[0]
                ax.errorbar(ts, posi_mu, yerr=posi_sig, label='[Tested positive]', errorevery=errorevery,
                            c=self.color_different_scenarios[counts], linestyle='--', elinewidth=0.8)
            else:


                ax.errorbar(ts, line_infected, yerr=error_infected, label=titles, errorevery=errorevery, elinewidth=0.8,
                    capsize=3.0, c=self.color_different_scenarios[counts], linestyle='--' if i == 0 and first_one_dashed else '-')
