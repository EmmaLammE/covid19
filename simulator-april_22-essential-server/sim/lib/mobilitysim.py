...
#     print(dur_mean_per_type)
    ...
    for k in range(len(mob_rate_per_type)):
        usual_sites_k=[]
        if (essential_workers) and (k == essential_type): # check if is essential worker, and if it is the work place
            usual_sites_k.append(essential_work_site) # each essential worker only goes to previously assigned work site
        else:            
            # All sites of type k
            s_args = np.where(site_type == k)[0]
    ...
    while t < max_time:
        ...
        # Shift time to next start of next visit
        t += rd.expovariate(tot_mob_rate)
#         print(site_type_prob,dur,rd.expovariate(tot_mob_rate))
        # Increment id
        id += 1
...
class MobilitySimulator:
    ...
    def __init__(self, delta, home_loc=None, people_age=None, site_loc=None, site_type=None,
        ...
        real = (home_loc is not None and people_age is not None and site_loc is not None and site_type is not None and
                ...
                tile_site_dist is not None and variety_per_type is not None and essential_workers is not None and essential_mob_rate_per_type is not None and essential_dur_mean_per_type is not None and essential_work_site is not None and essential_type is not None) 
