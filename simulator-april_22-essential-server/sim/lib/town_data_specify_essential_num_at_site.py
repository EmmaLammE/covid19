...
def generate_population(bbox, population_per_age_group, density_files=None, tile_level=16, seed=None, density_site_loc=None,essential_worker_per_site=None,site_type=None,essential_type=None,essential_distribution=None): 
    ...
    for _, t in tiles.iterrows():
        ...
        if essential_worker_per_site is not None:
#             essential_workers+=[(np.random.rand()<essential_prop_per_age_group[new_people_ages[i]]) for i in range(len(new_people_ages))]
            # assign each essential site type essential workers
            essential_site_ind = []
            cum_prob_essential_workers_per_site = []
            num_essential_site_type = 0
            num_essential_workers = 0             
            for i in range(len(site_type)):
                if site_type[i]==essential_type:
                    num_essential_site_type += 1
                    essential_site_ind += [i] # site index for each essential site
            num_essential_workers = essential_worker_per_site * num_essential_site_type
            ...
            prob_essential_workers_per_site = np.array([1/num_essential_site_type]*num_essential_site_type) # to be discussed
            ...
            for i in range(len(new_people_ages)):
                a = np.random.rand()
                for j in range(len(new_people_ages)):
    ...
    return home_loc, people_age, home_tile, tile_loc, essential_workers, num_essential_workers, essential_work_site
